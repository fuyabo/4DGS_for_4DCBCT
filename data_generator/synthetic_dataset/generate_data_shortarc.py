...
def main(args):
    ...
    # Load volume
    vol = np.load(os.path.join(vol_path, f'ct_50.npy')).astype(np.float32)
    # Generate training projections
    limited_angle_degree = 90
    projs_train_angles = (
            np.linspace(0, limited_angle_degree / 180 * np.pi, limited_angle_degree + 1)[:-1]
