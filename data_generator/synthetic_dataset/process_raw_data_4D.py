...
def main(args):
    ...
    for i_case in pbar:
        ...
        for t in range(10):
            ...
            raw_path = case_info["raw_path"]
            t_path = osp.join(raw_path, f"4DCT_{t_t}%")

            case_output_path = osp.join(output_path, f"{case_output_name}_{t_t}.npy")
            if not osp.exists(case_output_path):
                if file_type == "dcm":
                    ct_folder = next((f for f in os.scandir(t_path) if f.is_dir() and f.name.startswith('CT')), None)
                    vol_out = process_dcm(osp.join(t_path, ct_folder), target_size)
                elif file_type == "raw":
