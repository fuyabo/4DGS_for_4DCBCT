...
def init_pcd_grid(
    ...
    if True:
        ...
        edge_mask = gradient_magnitude > 3.5
...
def init_pcd(
    ...
    if recon_method == "random":
        ...
        vol = recon_volume(projs, angles, copy.deepcopy(geo), recon_method)
        # vol = mask_cylinder(vol, 5)
        # vol = np.load('data/spare_dataset/cone/vol_gt_00.npy')
        # for i in range(10):
        #     t_t = str(i*10).zfill(2)
        #     np.save( f"vol_gt_{t_t}.npy", vol)
        # np.save("fdk.npy", vol)
        show_one_volume(vol)
...
if __name__ == "__main__":
    ...
    args.output = os.path.join(args.data, "init_{}.npy".format(os.path.basename(args.data)))
    print("processing {}".format(args.data))
