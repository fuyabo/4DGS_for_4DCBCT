...
def readBlenderInfo(path, eval):
    ...
    # if with detector offset, assuming the detector is larger and include the offseted region and after splatting, crop
    meta_data["hasOffset"] = False
    if any(meta_data["scanner"]["offDetector"]):
        meta_data["hasOffset"] = True
        offset_in_pix = np.array(meta_data["scanner"]["offDetector"]) / np.array(meta_data["scanner"]["dDetector"])
        expand_detector_by_pixels = np.ceil(offset_in_pix) * 2
        expand_detector_by_mm = expand_detector_by_pixels * np.array(meta_data["scanner"]["dDetector"])
        meta_data["scanner"]["sDetector"] = list(np.array(meta_data["scanner"]["sDetector"]) + expand_detector_by_mm[[1, 0]])
        meta_data["scanner"]["nDetector"] = list(np.array(meta_data["scanner"]["nDetector"]) + expand_detector_by_pixels[[1, 0]])
        offset_in_pix[1] = -offset_in_pix[1]
        meta_data["scanner"]["crop_ul"] = list(np.array(expand_detector_by_pixels[[1, 0]])/2 + offset_in_pix[[1, 0]])
    ...
    # create gt for each phase and the mean
    vol_gt = {}
    vol_gt['mean'] = torch.from_numpy(np.load(osp.join(path, meta_data["vol"]))).float().cuda() # comment when you are running init_pcd, since cgls recon is not available yet
